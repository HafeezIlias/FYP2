...
// Forward declaration for LED indicators
void indicateTransmission();

// Global variables for LoRa status
extern String loraStatus;
extern int packetCount;
bool loraInitialized = false;  // Track if LoRa was successfully initialized
...
bool isLoRaHealthy() {
  return loraInitialized && (loraStatus != "Failed" && loraStatus != "Error");
}

void sendLoRaPacket(String jsonData) {
  if (!loraInitialized) {
    Serial.println("LoRa not initialized - cannot send packet");
    loraStatus = "Failed";
    return;
  }
